...
Public Class frmequipment
    ...
    Private Function saveCheck() As Boolean
        ...
        If String.IsNullOrEmpty(cmbGroup.Text) Then lblMessage.Text = "Equipment Group is Required" : cmbGroup.Focus() : rtnval = False
        If String.IsNullOrEmpty(txtInternalcode.Text) Then lblMessage.Text = "Truck Code is Required" : txtInternalcode.Focus() : rtnval = False
    ...
    Private Sub UpdateRecord()
        ...
        Try
            ...
            With adapter
                sql_String = "select * from [equipment] where recid=" & recID
            ...
            End If
            dr.Item("eqpGroup") = cmbGroup.Tag
            dr.Item("internalCode") = txtInternalcode.Text
            dr.Item("name") = txtName.Text
            dr.Item("vinNumber") = txtvin.Text
            dr.Item("model") = txtmodel.Text
            dr.Item("platenumber") = txtplate.Text
            If RadioButton1.Checked Then dr.Item("IsCompanyOwn") = 1
            If RadioButton2.Checked Then dr.Item("IsCompanyOwn") = 0
            dr.Item("notes") = txtnote.Text
    ...
    Private Sub btnSubmit_Click(sender As Object, e As EventArgs) Handles btnSubmit.Click
        ...
        Select Case cmbSearchBy.SelectedIndex
            Case 0 : vfldName = "e2.name"
            Case 1 : vfldName = "e2.etype"
                'Case 2 : vfldName = "[group]"
        End Select

        Try
            sql_String = " select e2.name [Group],e2.etype [Type], e1.internalcode [Code], e1.name [Name], e1.platenumber, e1.vinnumber [Chassis#],e1.model,"
            sql_String += " case iscompanyown when 1 then 'Company' else 'Rental' end [OwnerShip], br.brcode [Branch], e1.isactive [Active],e1.notes [Note],e1.recid,e2.recid [grprecid]"
            sql_String += " from equipment e1 inner join equipmentgroup e2 on e1.eqpgroup=e2.recid"
            sql_String += " inner join branch br on e1.branchid=br.recid where e1.recid>0 and br.recid=" & appUserInfo.BranchId
            ...
            If dsx.Tables(0).Rows.Count > 0 Then
                ...
                RadGridView2.Columns("grprecid").IsVisible = False
                ' dsPrintSearch = dsx
            End If
        Catch ex As Exception
    ...
    Private Sub getItemData(mRecID As Integer)
        ...
        Dim ds As New DataSet
        Call clearForm()
        sql_String = " select e2.name [Group], e1.internalcode [Code], e1.name [Name], e1.platenumber [Plate#], e1.vinnumber [Chassis#],e1.model,"
        sql_String += "  iscompanyown [OwnerShip], br.brcode [Branch], e1.isactive,e1.notes,e1.recid,e2.recid [grprecid]"
        sql_String += " from equipment e1 inner join equipmentgroup e2 on e1.eqpgroup=e2.recid"
        sql_String += " inner join branch br on e1.branchid=br.recid"
        sql_String += " where e1.recid=" & mRecID
        ...
        Try
            ...
            If ds.Tables(0).Rows.Count > 0 Then
                With ds.Tables(0).Rows(0)
                    ...
                    cmbGroup.Text = .Item("Group")
                    cmbGroup.Tag = .Item("grprecid")
                    txtInternalcode.Text = .Item("code")
                    txtName.Text = .Item("name")
                    txtvin.Text = .Item("Chassis#")
                    txtmodel.Text = .Item("model")
                    txtplate.Text = .Item("Plate#")
                    txtnote.Text = .Item("notes")
                    If .Item("OwnerShip") = 0 Then RadioButton1.Checked = True
                    If .Item("OwnerShip") = 1 Then RadioButton2.Checked = True
    ...
    Private Sub initFormLoad()
        ...
        frmWaitForm.Refresh()
        Me.Cursor = Cursors.WaitCursor
        sql_String = "select Name, ShortName, recid from [equipmentgroup] where isActive=1 and recid>0 order by Name"
        Call fillCombo(cmbGroup, sql_String, 600, 0, 300, 100)
        Me.Cursor = Cursors.Default
        frmWaitForm.Close()
    End Sub
    ...
    Private Sub txtNameAR_TextChanged(sender As Object, e As EventArgs) Handles txtInternalcode.TextChanged
    ...
    Private Function ExportToExcel() As String
        Try
            ...
            Dim destinationFile As String = copyTemplate("iffReportTemplate.xls", "Equipment List")
            ...
            xlWorkSheet.Cells(6, 1) = "Equipment List"
